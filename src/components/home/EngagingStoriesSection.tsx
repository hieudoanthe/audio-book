...
import {
  mockAllEngagingStories as allEngagingStories,
  mockEngagingStoriesCategories as categories,
  mockSingleEngagingStoriesCategory as singleEngagingStoriesCategory,
} from "@/mocks/books";
...
export default function EngagingStoriesSection() {
  const [activeCat, setActiveCat] = useState<string>(CATEGORIES[0]);
  const data = useMemo(() => {
    if (activeCat === "Tất cả") return allEngagingStories;
    return singleEngagingStoriesCategory[activeCat] ?? allEngagingStories;
  }, [activeCat]);
      ...
      >
        {data.map((b) => (
          <BookCard
            ...
            imageUrl={b.imageUrl}
            onClick={() => {
              window.location.href = `/stories/${b.id}`;
            }}
          />
        ))}
      </Carousel>
