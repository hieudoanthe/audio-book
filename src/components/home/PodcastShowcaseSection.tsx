...
import {
  mockAllPostcardShowcase as allPostcardShowcase,
  mockPostcardShowcaseCategories as categories,
  mockSinglePostcardShowcaseCategory as singlePostcardShowcaseCategory,
} from "@/mocks/books";
...
export default function PodcastShowcaseSection() {
  ...
  const data = useMemo(() => {
    if (activeCat === "Tất cả") return allPostcardShowcase;
    return singlePostcardShowcaseCategory[activeCat] ?? allPostcardShowcase;
  }, [activeCat]);
      ...
      >
        {data.map((b) => (
          <BookCard
            ...
            imageUrl={b.imageUrl}
            onClick={() => {
              window.location.href = `/podcast/${b.id}`;
            }}
          />
        ))}
      </Carousel>
